...
import type { TokenCollection, TokenData, TokenGroup } from "./types";
import { isAlias, mapScopes, parseColor } from "./utils";
...
export async function importTokens(data: unknown): Promise<void> {
	...
	for (const [collectionName, collectionData] of Object.entries(tokenData)) {
		...
		// Try to find collection by ID first (if provided), then fall back to name
		let collection = typedCollectionData.$collectionId
			? allCollections.find((c) => c.id === typedCollectionData.$collectionId)
			: undefined;
...
async function traverseTokens({
	...
	aliases,
	key,
}: TraverseTokensParams): Promise<void> {
	for (const [tokenKey, tokenValue] of Object.entries(object)) {
		...
		if (typedValue.$value !== undefined) {
			...
			if (isAlias(typedValue.$value)) {
				...
				console.log(
					`🔍 Alias detected: ${fullKey} → ${typedValue.$value} → ${valueKey}`,
				);
				...
				console.log(
					`✅ Creating token: ${fullKey} (${tokenType}) = ${typedValue.$value}`,
				);
				try {
					tokens[fullKey] = await createVariable(
						collection,
						modeId,
						fullKey,
						typedValue,
...
async function createVariable(
	...
	const figmaType =
		typeMap[tokenType] || ("STRING" as VariableResolvedDataType);

	// Try to find existing variable by ID first (if provided), then by name
	const allVariables = await figma.variables.getLocalVariablesAsync(figmaType);
	let variable: Variable | undefined;

	if (token.$extensions?.figma?.variableId) {
		variable = allVariables.find(
			(v) => v.id === token.$extensions.figma.variableId,
		);
		if (variable) {
			console.log(
				`  🔄 Found existing variable by ID: ${name} (${variable.id})`,
			);
		}
	}
	...
	if (!variable) {
		try {
			...
			const allTypes: VariableResolvedDataType[] = [
				"COLOR",
				"STRING",
				"FLOAT",
				"BOOLEAN",
			...
			for (const searchType of allTypes) {
				const variables =
					await figma.variables.getLocalVariablesAsync(searchType);
				variable = variables.find(
					(v) => v.name === name && v.variableCollectionId === collection.id,
				);
				if (variable) {
					console.log(
						`  🔄 Found existing variable with type ${searchType}: ${name}`,
					);
					break;
				}
			}
...
async function createVariableAlias(
	...
	const figmaType = (typeMap[tokenType] ||
		targetVariable.resolvedType) as VariableResolvedDataType;

	// Try to find existing variable by ID first (if provided), then by name
	const allVariables = await figma.variables.getLocalVariablesAsync(figmaType);
	let variable: Variable | undefined;

	if (token && token.$extensions?.figma?.variableId) {
		variable = allVariables.find(
			(v) => v.id === token.$extensions.figma.variableId,
		);
		if (variable) {
			console.log(
				`  🔄 Found existing alias variable by ID: ${name} (${variable.id})`,
			);
		}
	}
...
async function processAliases({
	...
	while (aliasArray.length && generations > 0) {
		for (let i = 0; i < aliasArray.length; i++) {
			...
			if (!targetVariable) {
				console.log(
					`  🔎 Searching for cross-collection alias: ${valueKey} (type: ${type})`,
				);
				const typeMap: Record<string, VariableResolvedDataType> = {
					color: "COLOR",
					string: "STRING",
				...
				const figmaType = (typeMap[type] ||
					"COLOR") as VariableResolvedDataType;
				const allVariables =
					await figma.variables.getLocalVariablesAsync(figmaType);
				console.log(
					`  📋 Found ${allVariables.length} variables of type ${figmaType}`,
				);
				console.log(
...
async function setModeValues({
	modeId,
	object,
	tokens,
	key,
}: SetModeValuesParams): Promise<void> {
	for (const [tokenKey, tokenValue] of Object.entries(object)) {
		// Skip meta fields
		if (tokenKey.charAt(0) === "$") {
			continue;
				...
				if (isAlias(typedValue.$value)) {
					...
					if (!targetVariable) {
						...
						if (foundVariable) {
							targetVariable = foundVariable;
							console.log(
								`  🔗 [Dark Mode] Found cross-collection reference: ${valueKey}`,
							);
							// Store it for future use
							tokens[valueKey] = targetVariable;
						} else {
							console.log(
								`  ❌ [Dark Mode] Cross-collection alias not found: ${valueKey}`,
							);
						}
					}
					...
					if (targetVariable) {
						...
						console.log(
							`  ⚠️ [Dark Mode] Cannot resolve alias for ${fullKey} → ${valueKey}`,
						);
					}
				} else {
